...
export default function ConsensusDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setClosePriceList(result.data.chartInfo.map((element, index) => parseInt(element.closePrice, 10)));
    ...
    <div className="px-[180px] mb-20">
      <div>
        <div className="flex justify-center items-center gap-2 mt-4">
          <p className="text-subheading text-body1">{location.state.companyName.split(' ')[0]}</p>
          ...
          >
            {location.state.type === 'buy' ? 'Buy' : location.state.type === 'sell' ? 'Sell' : '기본'}
          </button>
        </div>
        ...
        <div className="w-4/5 flex-col">
          <div className="flex justify-between items-end">
            ...
            <div>
              ...
              <p className="text-spray-350 text-heading3 font-bold">{Number(currentPrice).toLocaleString()}</p>
            </div>
          </div>
          <div className="flex justify-between items-end mt-4">
            ...
            <div>
              ...
              <p className="text-pink-100 text-heading3 font-bold">{Number(targetPrice).toLocaleString()}</p>
            </div>
          </div>
              ...
              {financialInfo[0] &&
                Object.keys(financialInfo[0]).map((key) => (
                  <div key={key} className="flex justify-between mb-2">
                    ...
                    <p className="text-tuatara-300 font-regular">{financialInfo[0][key]}</p>
                  </div>
                ))}
            </div>
            ...
            <div className="w-full">
              {financialInfo[1] &&
                Object.keys(financialInfo[1]).map((key) => (
                  <div key={key} className="flex justify-between mb-2">
                    <p className="text-tuatara-300 font-bold">{key}</p>
                    <p className="text-tuatara-300 font-regular">{financialInfo[1][key]}</p>
                  </div>
                ))}
            </div>
          </div>
          <p className="mt-4 text-mini text-center">[{financialInfo[2].년도} 기준]</p>
        </div>
      </div>
